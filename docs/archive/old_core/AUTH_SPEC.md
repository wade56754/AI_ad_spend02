...
> **规范级别**: 🔴 强制执行（PR必查）
> **文档定位**: 开发/测试/前端/后端/架构都可单独依赖本文件完成认证授权实现
...
| [2. 用户模型](#2-用户模型user-model-sot) | users表结构、5大角色 | 后端开发、DBA |
| [3. 认证机制](#3-认证机制authentication) | Supabase Auth、JWT Token | 前端+后端开发 |
| [4. Auth状态机](#4-auth状态机) | Token生命周期、状态流转 | 后端开发 |
| [5. 授权机制](#5-授权机制authorization) | RBAC、SOD、数据隔离 | 后端开发 |
| [6. Auth API SoT](#6-auth-api-sot) | 登录/登出/刷新端点 | 全栈开发 |
| [7. 错误码](#7-错误码auth错误码sot) | AUTH_*错误码 | 全栈开发 |
| [8. 审计要求](#8-审计要求audit-specification) | 审计日志规范 | 后端开发、安全 |
| [9. 安全策略](#9-安全策略security-rules) | Zero Trust原则 | 全员 |
| [10. 测试矩阵](#10-测试矩阵auth-test-plan) | 测试用例清单 | 测试工程师 |
...
**本文档是AI_AD_SYSTEM认证授权领域的唯一真相源**，负责定义：
...
| 领域 | 唯一真相源 | 仲裁规则 | 示例 |
|-----|-----------|---------|------|
| **数据库字段** | DATA_SCHEMA.md v5.1 | 字段名/类型/约束以DATA_SCHEMA为准 | `users.role`的CHECK约束 |
...
| **状态流转** | STATE_MACHINE.md v2.5 | 业务状态以STATE_MACHINE为准 | 项目/日报状态机 |
| **Token生命周期** | AUTH_SPEC.md v2.0 (本文档) | Token TTL/刷新策略以本文档为准 | Access Token 1小时 |
| **认证流程** | AUTH_SPEC.md v2.0 (本文档) | Token验证/权限校验链路以本文档为准 | JWT验证逻辑 |

**冲突处理规则**:
- ✅ 如果DATA_SCHEMA与AUTH_SPEC字段定义冲突 → **以DATA_SCHEMA为准，修改AUTH_SPEC**
- ✅ 如果BUSINESS_RULES与AUTH_SPEC权限规则冲突 → **以BUSINESS_RULES为准，修改AUTH_SPEC**
- ✅ 如果ERROR_CODES与AUTH_SPEC错误码冲突 → **以ERROR_CODES为准，修改AUTH_SPEC**
...
**引用**: DATA_SCHEMA.md v5.1 第3.1.1节
...
CREATE TABLE users (
    ...
    account_manager_id UUID REFERENCES users(id),  -- 投手关联的户管
...
interface UserPreferences {
  ...
  dashboard_layout: string;           // 默认: '{}' (JSON字符串)
...
interface ProfileMetadata {
  ...
  custom_fields?: Record<string, any>;  // 自定义扩展
}
```
...
| 字段 | 类型 | 默认值 | 说明 | 状态组合 |
|------|------|--------|------|---------|
| `is_active` | BOOLEAN | `true` | 账号可用性 | `false`时禁止登录（返回`AUTH_002`） |
| `is_verified` | BOOLEAN | `false` | 资料验证状态 | 由admin手动验证，不影响登录 |
...
| `false` | `true` | ❌ 禁止 | ❌ 禁止 | 已禁用账户 |
| `false` | `false` | ❌ 禁止 | ❌ 禁止 | 已禁用+未验证 |

### 2.5 last_login_at / last_login_ip 规则

- **更新时机**: 每次成功调用`POST /api/v1/auth/login`后立即更新
- **时区要求**: 统一使用UTC时区（TIMESTAMPTZ）
- **IP格式**: 支持IPv4/IPv6，最大长度45字符
...
```json
{
  ...
  "exp": 1705914000,        // 过期时间 (iat + 3600秒 = 1小时)
  "aud": "authenticated",
...
async def get_current_user(
    ...
    if not token:
        raise HTTPException(
            ...
            detail={"success": False, "message": "未提供认证令牌", "code": "AUTH_400"}
        )
    ...
    try:
        ...
        if "expired" in str(e).lower():
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail={"success": False, "message": "令牌已过期", "code": "AUTH_402"}
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail={"success": False, "message": "无效的认证令牌", "code": "AUTH_401"}
            )
...
class DailyReportService:
    def create_report(self, report: DailyReportCreate, user: Dict):
        ...
        new_report = DailyReport(
            ...
            created_by=user["user_id"],  # ← 从Token提取，前端无法伪造
...
| Token类型 | 默认TTL | 可配置范围 | 配置位置 |
|----------|--------|-----------|---------|
| **Access Token** | 1小时（3600秒） | 5分钟 - 24小时 | Supabase Dashboard → Auth Settings |
| **Refresh Token** | 30天（2592000秒） | 7天 - 365天 | Supabase Dashboard → Auth Settings |

**Supabase配置路径**:
```
项目Dashboard → Authentication → Settings → JWT Settings
- JWT expiry limit: 3600 (秒)
...
- ✅ 如果旧Refresh Token被重复使用 → 返回`AUTH_003`（令牌已被撤销）
...
| **账户被禁用** | `is_active=false` | 所有Session | `AUTH_002` |
...
|-----|------|------|------|
| `active` | Token有效且未过期 | ❌ | - |
| `expired` | Token自然过期（exp < NOW()） | ✅ | ❌ 不可逆 |
| `invalidated` | 主动失效（登出、强制下线） | ✅ | ❌ 不可逆 |

**引用**: STATE_MACHINE.md v2.5（本状态机为Auth专用，不在STATE_MACHINE.md中）

### 4.2 状态流转白名单表

| 当前状态 | 事件 | 目标状态 | 触发条件 | 操作者 |
|---------|------|---------|---------|--------|
| `active` | `refresh` | `active` | 有效的Refresh Token | 用户 |
| `active` | `expire` | `expired` | `exp < NOW()` | system（自动） |
| `active` | `logout` | `invalidated` | 用户主动登出 | 用户 |
| `active` | `security_invalid` | `invalidated` | 账户禁用、风控下线 | admin/system |
| `expired` | `refresh` | ❌ 禁止 | 过期后不可刷新 | - |
| `invalidated` | `refresh` | ❌ 禁止 | 已失效不可恢复 | - |
    ...
    def check_token_state(self, token: str) -> str:
        ...
        try:
            ...
            if session.expires_at < now:
                return "expired"  # 已自然过期

            # Step 5: 检查用户状态
            user = self.db.query(User).filter(User.id == session.user_id).first()
            if not user or not user.is_active:
                return "invalidated"  # 账户已禁用
...
**引用**: SYSTEM_OVERVIEW.md v2.0 - 第6章, BUSINESS_RULES.md v3.1 - BR-USER-001
...
| 业务流程 | 提交角色 | 审核/审批角色 | SOD规则 | 违反错误码 |
|---------|---------|--------------|--------|-----------|
| **日报审核** | media_buyer | data_operator | 提交人 ≠ 审核人 | `BIZ_001` |
| **充值审批** | media_buyer/account_manager | data_operator (初审) + finance (终审) | 申请人 ≠ 审核人 ≠ 审批人 | `BIZ_001` |
| **对账确认** | data_operator | finance | 提交人 ≠ 确认人 | `BIZ_001` |
...
class DailyReportService:
    def approve_report(self, report_id: int, user: Dict) -> DailyReport:
        ...
        if not report:
            raise ResourceNotFoundException(code="BIZ_002", message="日报不存在")
...
| **日报管理 (Daily Reports)** |
| GET /daily-reports | ✅ 全部 | 🔍 全部 | 🔍 全部 | 🔍 仅所管项目 | 🔍 仅自己提交的 |
| POST /daily-reports | ✅ | ❌ | ❌ | ❌ | ✅ |
| PUT /daily-reports/{id} | ✅ | ❌ | ❌ | ❌ | ✅ (仅raw_submitted) |
| POST /daily-reports/{id}/approve | ✅ | ❌ | ✅ (禁止审核自己提交的) | ❌ | ❌ |
| POST /daily-reports/{id}/confirm-final | ✅ | ❌ | ✅ | ❌ | ❌ |
| **充值管理 (Topup Requests)** |
| GET /topup-requests | ✅ 全部 | ✅ 全部 | 🔍 全部 | 🔍 仅所管项目 | 🔍 仅自己申请的 |
| POST /topup-requests | ✅ | ❌ | ❌ | ✅ | ✅ |
...
**引用**: SYSTEM_OVERVIEW.md v2.0 - 第6.2节

#### 5.3.1 过滤规则矩阵

| 角色 | 模块 | SQL过滤条件 | 说明 |
|-----|------|-----------|------|
| **admin** | 所有模块 | `WHERE 1=1` | 无过滤 |
| **finance** | 充值/账本/对账 | `WHERE 1=1` | 无过滤 |
...
| **media_buyer** | 账户 | `WHERE assigned_to = :user_id` | 仅分配给自己的 |
...
| `data.expires_in` | integer | Access Token有效期（秒），默认3600 |
...
**引用**: ERROR_CODES_SOT.md v2.1 - 第4.1节
...
| `AUTH_003` | 401 | 令牌已被撤销 | 主动登出、多端登录、强制下线 | USED |
| `AUTH_004` | 404 | 用户不存在或已被删除 | `users`表无记录 | USED |
| `AUTH_005` | 401 | 令牌刷新失败 | Refresh Token无效、过期或已被使用 | RESERVED |
| `AUTH_100` | 400 | 邮箱已被注册 | 注册时邮箱重复 | USED |
| `AUTH_400` | 401 | 未提供认证令牌 | 缺少`Authorization` Header | USED |
| `AUTH_401` | 401 | 无效的认证令牌 | Token格式错误、签名验证失败 | USED |
| `AUTH_402` | 401 | 令牌已过期 | Token超过TTL | USED |
| `AUTH_500` | 403 | 权限不足 | 角色不满足权限要求 | USED |
| `AUTH_501` | 403 | 角色权限不足 | 未分配有效角色 | RESERVED |

### 7.2 业务错误码（BIZ_）

| 错误码 | HTTP状态码 | 消息 | 触发场景 | 状态 |
|--------|-----------|------|----------|------|
| `BIZ_001` | 400 | 无效的操作 | 违反业务规则（如SOD） | USED |
...
**引用**: DATA_SCHEMA.md v5.1 - 第3.1.4节 (audit_logs表)
...
CREATE TABLE audit_logs (
    ...
    module VARCHAR(100) NOT NULL,           -- 业务模块名（如 "auth", "users", "daily_reports"）
    ...
    payload_before JSONB,                   -- 操作前数据
    payload_after JSONB,                    -- 操作后数据
...
await apiFetch('/api/v1/daily-reports/123', {
  ...
  body: JSON.stringify({
    conversions_raw: 100,
    created_by: 'other-user-id'  // ← 前端可伪造
  })
});
```
...
| 测试用例编号 | 测试场景 | 输入 | 期望输出 | 优先级 |
|------------|---------|------|---------|--------|
| TC-AUTH-001 | 正常登录 | 正确的email+password | HTTP 200, 返回token | P0 |
| TC-AUTH-002 | 密码错误 | 错误的password | HTTP 401, `AUTH_001` | P0 |
| TC-AUTH-003 | 邮箱不存在 | 不存在的email | HTTP 401, `AUTH_001` | P0 |
| TC-AUTH-004 | 账户已禁用 | `is_active=false`的用户 | HTTP 403, `AUTH_002` | P0 |
...
| TC-AUTH-008 | Token未过期 | 有效的Access Token | 正常返回业务数据 | P0 |
...
| TC-AUTHZ-005 | SOD检查 - 自我审核（禁止） | data_operator | 审核自己提交的日报 | HTTP 400, `BIZ_001` | P0 |
...
| 测试用例编号 | 测试场景 | 输入 | 期望输出 | 优先级 |
|------------|---------|------|---------|--------|
| TC-SESSION-001 | 多设备同时登录 | 用户在设备A和设备B登录 | 两个设备都可正常访问 | P0 |
| TC-SESSION-002 | 设备A登出 | 设备A调用logout | 设备A无法访问，设备B正常 | P0 |
| TC-SESSION-003 | 强制下线所有设备 | Admin调用force_logout | 所有设备均返回`AUTH_003` | P0 |
...
| TC-SOD-003 | 数据员提交+同一数据员审核（禁止） | 数据员A提交并尝试审核 | HTTP 400, `BIZ_001` | P0 |
| TC-SOD-004 | Admin绕过SOD（允许+审计） | Admin强制审核自己提交的日报 | HTTP 200, 审计日志含`SOD_BYPASS` | P0 |
...
| SoT文档 | 版本 | 引用章节 | 引用次数 |
|---------|------|---------|---------|
| **DATA_SCHEMA.md** | v5.1 | 3.1.1 users表, 3.1.3 user_sessions表, 3.1.4 audit_logs表 | 15+ |
...
| **STATE_MACHINE.md** | v2.5 | 角色定义、操作者规则 | 3+ |
| **SYSTEM_OVERVIEW.md** | v2.0 | 第6章 - 核心角色与职责 | 5+ |
| **API_SOT.md** | v9.0 | 第2章 - 统一规范（认证与授权） | 4+ |
...
| v2.0 | 2025-01-22 | **正式SoT版**<br>- 补全Auth状态机定义<br>- 补全4个Auth API端点完整规范<br>- 补全SOD规则矩阵与实现<br>- 补全数据隔离Service层实现<br>- 补全Token生命周期（TTL、Rotation、失效策略）<br>- 补全安全策略（Zero Trust、CSRF、重放攻击防护）<br>- 补全测试矩阵（7大类测试场景）<br>- 补全审计日志规范 | 系统架构团队 |
