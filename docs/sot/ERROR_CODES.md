...
| `REC_` | 对账错误 | 对账守恒校验、差异单处理相关错误 | 400, 403 | 6 |
| `SET_` | 结算错误 | 结算规则配置、计算相关错误 | 400 | 4 |
...
> **引用**: `BUSINESS_RULES.md` BR-REC-001 ~ BR-REC-004

**业务背景**: 对账中控系统负责验证"对账守恒公式"，追踪差异单的生命周期。

| 错误码 | 消息 | HTTP | 触发场景 | 状态 |
|--------|------|------|----------|------|
| `REC_001` | 对账守恒校验失败 | 400 | 守恒公式差异超阈值（>¥100） | USED |
| `REC_002` | 快照数据缺失 | 400 | 对账期间缺少余额快照 | USED |
| `REC_003` | 差异单状态流转非法 | 400 | 违反对账差异单状态机规则 | USED |
| `REC_004` | 差异单操作权限不足 | 403 | 非责任人尝试处理差异单 | USED |
| `REC_005` | SLA 已超时 | 400 | 差异单超过关闭期限 | USED |
| `REC_006` | 差异单未分配 | 400 | 尝试处理未分配的差异单 | USED |
...
# 场景1: 守恒校验失败
if abs(difference) > Decimal("100.00"):
    raise BusinessRuleException(
        message=f"对账守恒失败，差异 ¥{difference}",
        code=ReconciliationErrorCodes.CONSERVATION_FAILED.code  # REC_001
    )

# 场景2: 快照缺失
if not snapshot:
    raise BusinessRuleException(
        message=f"账户 {account_id} 在 {date} 缺少快照",
        code=ReconciliationErrorCodes.SNAPSHOT_MISSING.code  # REC_002
    )

# 场景3: 非责任人操作
if issue.assigned_to != current_user.id:
    raise AuthorizationException(
        message="只有责任人可以处理此差异单",
        code=ReconciliationErrorCodes.PERMISSION_DENIED.code  # REC_004
    )
```
...
> **OpenSpec Change**: `add-reconciliation-control-center`
> **引用**: `BUSINESS_RULES.md` BR-SET-001 ~ BR-SET-003

**业务背景**: 支持三种结算类型（fixed/tiered/markup），需要校验结算规则配置的有效性。

| 错误码 | 消息 | HTTP | 触发场景 | 状态 |
|--------|------|------|----------|------|
| `SET_001` | 结算规则配置无效 | 400 | tiered 阶梯不连续或格式错误 | USED |
| `SET_002` | 结算类型不支持 | 400 | 使用未定义的 settlement_type | USED |
| `SET_003` | 结算规则生效期冲突 | 400 | 同一项目存在重叠生效期 | USED |
| `SET_004` | 结算规则缺失 | 400 | tiered/markup 类型缺少规则配置 | USED |

**结算类型说明**:
- `fixed`: 固定单价，使用 `projects.unit_price` 计算
- `tiered`: 阶梯计价，支持 cumulative（累计）和 incremental（递增）两种模式
- `markup`: 加成计价，支持 percentage（百分比）和 fixed（固定值）两种模式

**使用示例**:
```python
from backend.core.error_codes import SettlementErrorCodes

# 场景1: 阶梯配置校验
def validate_tiered_config(config: dict):
    tiers = config.get("tiers", [])
    for i in range(1, len(tiers)):
        if tiers[i]["min"] != tiers[i-1]["max"] + 1:
            raise BusinessRuleException(
                message="阶梯配置不连续",
                code=SettlementErrorCodes.INVALID_CONFIG.code  # SET_001
            )

# 场景2: 结算类型校验
if settlement_type not in ("fixed", "tiered", "markup"):
    raise BusinessRuleException(
        message=f"不支持的结算类型: {settlement_type}",
        code=SettlementErrorCodes.UNSUPPORTED_TYPE.code  # SET_002
    )

# 场景3: 规则缺失
if project.settlement_type != "fixed" and not project.settlement_rules_id:
    raise BusinessRuleException(
        message="非fixed结算类型必须关联结算规则",
        code=SettlementErrorCodes.RULE_MISSING.code  # SET_004
    )
```
