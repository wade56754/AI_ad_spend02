...
| `reconciliation_batches.status` | `closed` | 对账批次关闭 |
...
## 14.4 【P0 必须补充】对账批次关闭条件

### 14.4.1 关闭前置条件

对账批次 `reconciliation_batches` 从 `reviewing` 流转至 `closed` 必须满足：
...
def close_reconciliation_batch(
    ...
    with transaction():
        ...
        audit_log(
            ...
            performed_by=operator.id,
            role=operator.role,
            payload_before={"status": "reviewing"},
...
| 场景 | 禁止原因 | 错误码 |
|------|---------|--------|
| 批次状态为 `draft` 或 `pending` | 未进入 reviewing 阶段 | `STATE_400` |
...
### 14.4.4 强制关闭机制（Admin 特权）

**场景**：紧急情况下需强制关闭批次（如系统故障、数据迁移）。

**允许条件**：
1. 仅 `admin` 可执行
2. 必须提供强制关闭理由（最少 20 字符）
3. 必须记录安全审计日志
4. 强制关闭后批次状态标记为 `closed_forced`（区别于正常 `closed`）

**实施代码**：

```python
def admin_force_close_batch(
    batch_id: int,
    reason: str,
    operator: AuthenticatedUser
) -> Result:
    ...
    with transaction():
        batch = get_reconciliation_batch(batch_id)
        batch.status = "closed"  # 当前不支持 closed_forced，使用 closed
        batch.force_closed = True  # 新增标记字段
        batch.force_close_reason = reason
        batch.updated_by = operator.id
        db.commit()
        ...
        audit_log(
            ...
            role="admin",
            reason=reason,
            payload_after={"status": "closed", "force_closed": True}
        )

        # 发送安全告警
        send_security_alert(
            level="HIGH",
            message=f"Admin {operator.username} 强制关闭对账批次 {batch_id}",
            reason=reason
        )
```
...
| `STATE_400` | 当前状态不允许关闭 | 400 | 状态非 reviewing |
...
| `PUT /reconciliation-batches/{id}/submit` | PUT | draft | pending | finance, admin | [AUDIT] | STATE_400, AUTH_500 |
| `PUT /reconciliation-batches/{id}/review` | PUT | pending | reviewing | finance, admin | [AUDIT] | STATE_400, AUTH_500 |
| `PUT /reconciliation-batches/{id}/close` | PUT | reviewing | closed | finance, admin | [AUDIT] | STATE_400, RECON_001/002/003 |
| `PUT /reconciliation-batches/{id}/force-close` | PUT | any (non-closed) | closed | admin | [AUDIT] 强制 | STATE_400, AUTH_500 |
