...
| 检查项 | 说明 | SoT 依据 |
|--------|------|----------|
| 状态枚举 | 禁止自定义，必须使用 8 状态机 | STATE_MACHINE.md v2.7 |
| 错误码 | 必须使用标准格式 `{"code": "XXX-NNN"}` | ERROR_CODES_SOT.md v2.1 |
| 账本操作 | 禁止直接修改 balance | LEDGER_SOT.md v1.2 |
| 数据模型 | 字段类型必须与 Schema 一致 | DATA_SCHEMA.md v5.3 |
...
```
┌─────────────────────────────────────────────────────────────┐
│  STATE_MACHINE.md v2.7  (状态机定义，最高优先级)              │
├─────────────────────────────────────────────────────────────┤
│  DATA_SCHEMA.md v5.3    (数据模型，23 张表)                  │
├─────────────────────────────────────────────────────────────┤
