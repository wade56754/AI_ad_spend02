...
| [2. 用户模型](#2-用户模型user-model-sot) | users表结构、角色定义 | 后端开发、DBA |
...
| `false` | `true` | ❌ 禁止 | ❌ 禁止 | 已禁用账号 |
| `false` | `false` | ❌ 禁止 | ❌ 禁止 | 已禁用未验证 |
...
**引用**: STATE_MACHINE.md v2.9（本状态机为Auth专用，不在STATE_MACHINE.md中）
...
|---------|---------|--------------|--------|-----------|
| **日报审核** | media_buyer | project_owner | 提交者 ≠ 审核者 | `BIZ_001` |
| **充值审批** | media_buyer/account_manager | project_owner (初审) + finance (终审) | 申请者 ≠ 审核者 ≠ 审批者 | `BIZ_001` |
| **对账确认** | project_owner | finance | 提交者 ≠ 确认者 | `BIZ_001` |
...
def admin_force_approve(self, report_id: int, user: Dict, reason: str):
    ...
    if not reason or len(reason) < 10:
        raise ValidationException(code="VALIDATION_001", message="必须填写详细原因（至少20字符）")
...
| TC-AUTHZ-005 | SOD检查 - 自我审核（禁止） | project_owner | 审核自己提交的日报 | HTTP 400, `BIZ_001` | P0 |
...
| SoT文档 | 版本 | 引用章节 | 引用次数 |
|---------|------|---------|---------|
| **DATA_SCHEMA.md** | v5.7 | §1.1.1 users表、§1.1.3 user_sessions表、§1.1.4 audit_logs表 | 15+ |
| **BUSINESS_RULES.md** | v5.0 | BR-AUTH-001~004, BR-USER-001~003, BR-FIN-002 | 12+ |
| **ERROR_CODES_SOT.md** | v2.1 | §4.1 认证授权类（AUTH_*）、§4.2 业务逻辑类（BIZ_*） | 8+ |
| **STATE_MACHINE.md** | v2.9 | 角色定义、操作者规则 | 3+ |
| **MASTER.md** | v4.8 | §2.4 - 核心角色与职责 | 5+ |
| **API_SOT.md** | v9.5 | §1.2 - 统一规范（认证与授权） | 4+ |
