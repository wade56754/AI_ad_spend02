"""
db_test_skill.py - 数据库测试 Skill

Phase 3: 从 agents/skills/db_test_skill.py 迁移

职责：
- 生成数据库测试提示词（给 Supabase MCP 使用）
- 不直接执行测试，只生成 prompt

基准对齐:
- AGENT_PLATFORM_MIGRATION_PLAN_v1.2.md Phase 3
- SoT Freeze v2.6 (DB_TEST_CASES, DB_INVARIANTS_SQL)
"""
...
def db_test_skill() -> Dict[str, Any]:
    ...
    """
    cases = read_optional(SOT_FILES.get("DB_TEST_CASES"))
    sql = read_optional(SOT_FILES.get("DB_INVARIANTS_SQL"))
    ...
    logger.debug(f"DB test prompt generated: sql={len(sql)} chars, cases={len(cases) if cases else 0} chars")
