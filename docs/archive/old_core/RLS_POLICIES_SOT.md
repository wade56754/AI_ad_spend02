...
**本文档是AI_AD_SYSTEM行级安全策略领域的唯一真相源**，负责定义：

- ✅ **RLS策略设计**: 所有业务表的行级访问控制策略
- ✅ **角色权限映射**: 5个角色的数据访问范围（admin/finance/data_operator/account_manager/media_buyer）
- ✅ **上下文函数**: 统一的JWT claims提取函数（`fn_current_user_id()`等）
- ✅ **安全基线**: Service层+RLS双重防护机制
- ✅ **测试规范**: RLS策略测试用例矩阵
...
AI_AD_SYSTEM 文档体系
│
├─ DATA_SCHEMA.md v5.1        ← 表结构、字段定义的唯一来源
├─ AUTH_SPEC.md v2.0          ← 角色定义、权限控制的唯一来源
├─ BUSINESS_RULES.md v3.1     ← 业务规则的唯一来源
├─ STATE_MACHINE.md v2.5      ← 状态流转的唯一来源
├─ LEDGER_SOT.md v1.1         ← 账本业务逻辑的唯一来源
│
└─ RLS_POLICIES_SOT.md v2.0 (本文档) ← RLS策略的唯一来源
```

### 1.3 禁止绕过规则

⚠️ **任何开发者不得绕过本文档设计RLS策略**

- ❌ **禁止**在SQL中编写"临时策略"而不更新本文档
- ❌ **禁止**在代码中硬编码角色权限逻辑绕过RLS
- ❌ **禁止**创建超级用户绕过RLS（除运维必要的postgres超级用户）
- ✅ **必须**所有RLS策略变更都先更新本文档，再执行SQL DDL
- ✅ **必须**所有策略都有对应的测试用例（第9章）
...
| 领域 | 唯一真相源 | 仲裁规则 | 示例 |
|-----|-----------|---------|------|
| **表结构/字段** | DATA_SCHEMA.md v5.1 | 字段名/类型/外键以DATA_SCHEMA为准 | `users.role` CHECK约束 |
...
| **状态流转** | STATE_MACHINE.md v2.5 | 状态机逻辑以STATE_MACHINE为准 | 粉数确认6状态流转 |
| **RLS策略** | RLS_POLICIES_SOT.md v2.0 (本文档) | RLS策略设计以本文档为准 | `policy_users_select_self` |
...
**核心原则**: 所有业务表必须启用RLS，即使table owner也受RLS限制
