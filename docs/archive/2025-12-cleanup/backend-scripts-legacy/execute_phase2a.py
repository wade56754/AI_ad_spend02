...
"""
Phase 2A 执行脚本（备用方案）
如果 Alembic 不可用，可以使用此脚本直接执行 SQL

注意：优先使用 Alembic！此脚本仅作为备用方案。
"""
...
print('=' * 60)
print('Phase 2A: 时间字段 timezone 修复')
print('=' * 60)
print(f'数据库: {dbname}')
print(f'主机: {host}')
print(f'端口: {port}')
...
# 用户确认
print('警告: 此操作将修改 7 个表共 14 个时间字段！')
print()
print('确认以下条件:')
print('1. 已通过 Supabase Dashboard 创建数据库快照')
print('2. DATABASE_URL 指向 Dev 环境（不是生产库）')
print('3. 已阅读 PHASE2A_EXECUTION_GUIDE.md')
print()

confirm = input('是否继续执行 Phase 2A？(yes/no): ')
...
try:
    ...
    print()
    print('下一步:')
    print('1. 执行完整的 Gate 验证: psql $DATABASE_URL -f phase2a_gate_verification.sql')
    print('2. 更新 SQLAlchemy 模型代码（参考 PHASE2A_EXECUTION_GUIDE.md）')
    print('3. 进入 3 天观察期（2025-11-18 至 2025-11-21）')
