...
'''
    ...
    def test_extract_json_with_array_values(self):
        """Should handle arrays within JSON."""
    ...
    def test_extract_first_valid_json(self):
        """Should extract first valid JSON when multiple are present."""
        text = '''
Invalid: {broken
Valid: {"first": true}
Also valid: {"second": true}
'''
        result = _extract_json(text)
        # Should find one of the valid JSONs
...
class TestClaudeCodeConfig:
    ...
    def test_timeout_is_reasonable(self):
        ...
        assert CLAUDE_CODE_CONFIG["timeout"] >= 60  # At least 1 minute
        assert CLAUDE_CODE_CONFIG["timeout"] <= 600  # At most 10 minutes
