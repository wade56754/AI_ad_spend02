...
class FinanceService:
    ...
    def get_profit_trend(
        ...
        # 根据粒度选择分组字段
        # 使用数据库无关的 extract 函数替代 PostgreSQL 专用的 date_trunc
        if granularity == "daily":
            ...
            # 使用 year + week 组合分组（跨数据库兼容）
            year_field = extract('year', DailyReport.report_date)
            week_field = extract('week', DailyReport.report_date)
            group_by = [year_field, week_field]
            query = self.db.query(
                func.min(DailyReport.report_date).label('period'),  # 使用每周最小日期作为周起始
                func.sum(DailyReport.conversions_final).label('conversions_final'),
                func.avg(DailyReport.unit_price).label('avg_unit_price'),
                func.sum(DailyReport.real_spend).label('real_spend'),
                year_field.label('_year'),
                week_field.label('_week'),
            ).join(
                AdAccount, DailyReport.ad_account_id == AdAccount.id
            )
            period_field = func.min(DailyReport.report_date)  # 用于排序
        else:  # monthly
            # 使用 year + month 组合分组（跨数据库兼容）
            year_field = extract('year', DailyReport.report_date)
            month_field = extract('month', DailyReport.report_date)
            group_by = [year_field, month_field]
            query = self.db.query(
                func.min(DailyReport.report_date).label('period'),  # 使用每月最小日期
                func.sum(DailyReport.conversions_final).label('conversions_final'),
                func.avg(DailyReport.unit_price).label('avg_unit_price'),
                func.sum(DailyReport.real_spend).label('real_spend'),
                year_field.label('_year'),
                month_field.label('_month'),
            ).join(
                AdAccount, DailyReport.ad_account_id == AdAccount.id
            )
            period_field = func.min(DailyReport.report_date)  # 用于排序
