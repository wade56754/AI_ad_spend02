...
| POST | `/api/v1/reconciliation/batches/{batch_id}/confirm` | 确认对账结果 | `finance`, `admin` | `reviewing` → `closed` | implemented |

**对账状态机**: `draft` → `pending` → `reviewing` → `closed`（来自 STATE_MACHINE.md 第11.1节）
