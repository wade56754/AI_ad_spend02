...
def test_be_agent_handle_request_missing_task():
    """Test that BEAgent returns error when task is missing."""
    be_agent = create_agent("be")
    response = be_agent.handle_request({"target_files": ["routers/topup.py"]})
    assert response["success"] is False
    assert response["error"] is not None
    assert "task" in response["error"].lower()


def test_be_agent_handle_request_invalid_target_files():
    """Test that BEAgent returns error when target_files is invalid."""
    be_agent = create_agent("be")

    # Test with missing target_files
    response = be_agent.handle_request({"task": "Implement topup list API"})
    assert response["success"] is False
    assert response["error"] is not None

    # Test with empty target_files list
    response = be_agent.handle_request({"task": "Implement topup list API", "target_files": []})
    assert response["success"] is False
    assert response["error"] is not None
