...
from backend.models import AdAccount, Project, Channel, User, AccountAlert, AccountNote, AccountStatusHistory
...
def ad_account_service(mock_db):
    ...
    with patch('backend.services.ad_account_service.AuditLogService', return_value=mock_audit_service):
...
class TestCreateAccount:
    ...
    async def test_create_account_success(
        ...
        # 执行创建
        with patch.object(ad_account_service, '_create_status_history', new=AsyncMock()):
            account = await ad_account_service.create_account(request, current_user_id=100)
    ...
    async def test_create_account_project_not_found(
        self, ad_account_service, mock_db
    ):
        """测试项目不存在时抛出异常"""
        # Mock查询返回None（项目不存在）
        mock_query = Mock()
    ...
    async def test_create_account_duplicate_account_id(
        self, ad_account_service, mock_db, sample_project, sample_channel, sample_account
    ):
        """测试账户ID已存在时抛出异常"""
        # Mock查询
        mock_query = Mock()
...
class TestGetAccounts:
    ...
    async def test_get_accounts_no_filters(self, ad_account_service, mock_db, sample_account):
        """测试无过滤条件查询所有账户"""
        mock_query = Mock()
        mock_query.count.return_value = 1
        mock_query.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [sample_account]
        mock_db.query.return_value = mock_query
    ...
    async def test_get_accounts_with_filters(self, ad_account_service, mock_db, sample_account):
        """测试带过滤条件查询"""
        mock_query = Mock()
    ...
    async def test_get_accounts_account_manager_role(self, ad_account_service, mock_db, sample_account):
        """测试客户经理角色只能看到自己项目的账户"""
        mock_query = Mock()
    ...
    async def test_get_accounts_pagination(self, ad_account_service, mock_db, sample_account):
        """测试分页功能"""
        mock_query = Mock()
...
class TestGetAccountById:
    ...
    async def test_get_account_by_id_success(self, ad_account_service, mock_db, sample_account):
        """测试成功获取账户详情"""
        mock_query = Mock()
    ...
    async def test_get_account_by_id_media_buyer_permission_denied(
        self, ad_account_service, mock_db, sample_account
    ):
        """测试投手无权访问其他人的账户"""
        sample_account.assigned_to = 50
        sample_account.assigned_user_id = 50  # 向后兼容属性
        mock_query = Mock()
        mock_query.filter.return_value.first.return_value = sample_account
        mock_db.query.return_value = mock_query
...
class TestUpdateAccount:
    ...
    async def test_update_account_success(self, ad_account_service, mock_db, sample_account):
        """测试成功更新账户信息"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock(return_value=sample_account)):
            request = Mock(spec=AdAccountUpdateRequest)
            request.dict.return_value = {"name": "新账户名称"}

            account = await ad_account_service.update_account(1, request, current_user_id=100)

            mock_db.commit.assert_called_once()
            assert account == sample_account

    async def test_update_account_no_changes(self, ad_account_service, mock_db, sample_account):
        """测试无变更时不记录审计日志"""
...
class TestUpdateAccountStatus:
    ...
    async def test_update_status_new_to_testing(self, ad_account_service, mock_db, sample_account):
        """测试new → testing状态转换"""
        ...
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock(return_value=sample_account)):
            with patch.object(ad_account_service, '_create_status_history', new=AsyncMock()):
                ...
                account = await ad_account_service.update_account_status(1, request, current_user_id=100)

                assert account.status == "testing"
                mock_db.commit.assert_called()

    async def test_update_status_testing_to_active(self, ad_account_service, mock_db, sample_account):
    ...
    async def test_update_status_active_to_suspended(self, ad_account_service, mock_db, sample_account):
    ...
    async def test_update_status_invalid_transition(self, ad_account_service, mock_db, sample_account):
        """测试非法状态转换抛出异常"""
        sample_account.status = "new"
        ...
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock(return_value=sample_account)):
            ...
            with pytest.raises(ValidationError) as exc:
                await ad_account_service.update_account_status(1, request, current_user_id=100)

            assert "不能从状态" in str(exc.value)

    async def test_update_status_archived_terminal_state(self, ad_account_service, mock_db, sample_account):
        """测试archived终态不能再转换"""
...
class TestUpdateAccountBudget:
    ...
    async def test_update_budget_daily_only(self, ad_account_service, mock_db, sample_account):
        """测试只更新日预算"""
        ...
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock(return_value=sample_account)):
            ...
            account = await ad_account_service.update_account_budget(1, request, current_user_id=100)

            assert account.daily_budget == Decimal("1500.00")
            mock_db.commit.assert_called()

    async def test_update_budget_total_only(self, ad_account_service, mock_db, sample_account):
        """测试只更新总预算"""
    ...
    async def test_update_budget_both(self, ad_account_service, mock_db, sample_account):
        """测试同时更新日预算和总预算"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock(return_value=sample_account)):
            request = Mock(spec=AdAccountBudgetUpdateRequest)
...
class TestGetAccountStatistics:
    ...
    async def test_get_statistics_basic(self, ad_account_service, mock_db, sample_account):
        """测试基础统计数据"""
        mock_query = Mock()
        mock_query.filter.return_value = mock_query
        mock_query.join.return_value = mock_query
        mock_query.count.side_effect = [10, 5, 2, 1, 2]  # total, active, suspended, dead, new
        ...
        mock_query.filter.return_value.count.side_effect = [3, 1]  # active_alerts, critical_alerts
...
class TestAccountAlerts:
    ...
    async def test_get_account_alerts_success(self, ad_account_service, mock_db, sample_alert):
        """测试获取账户预警列表"""
    ...
    async def test_get_account_alerts_with_filters(self, ad_account_service, mock_db, sample_alert):
        """测试带过滤条件的预警查询"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            mock_query = Mock()
            mock_query.filter.return_value = mock_query
    ...
    async def test_create_account_alert_success(self, ad_account_service, mock_db):
        ...
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            ...
            alert = await ad_account_service.create_account_alert(1, request, current_user_id=100)

            mock_db.add.assert_called_once()
            mock_db.commit.assert_called()
            mock_db.refresh.assert_called_once()

    async def test_update_account_alert_acknowledged(self, ad_account_service, mock_db, sample_alert):
        """测试确认预警"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            mock_query = Mock()
            ...
            alert = await ad_account_service.update_account_alert(1, request, current_user_id=100)

            assert alert.acknowledged_by == 100
            assert alert.acknowledged_at is not None
            mock_db.commit.assert_called()

    async def test_update_account_alert_resolved(self, ad_account_service, mock_db, sample_alert):
        """测试解决预警"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            mock_query = Mock()
            mock_query.filter.return_value.first.return_value = sample_alert
    ...
    async def test_update_account_alert_not_found(self, ad_account_service, mock_db):
        ...
        with pytest.raises(NotFoundError) as exc:
            await ad_account_service.update_account_alert(999, request, current_user_id=100)
...
class TestAccountNotes:
    ...
    async def test_get_account_notes_success(self, ad_account_service, mock_db, sample_note):
        """测试获取账户备注列表"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            mock_query = Mock()
            mock_query.filter.return_value = mock_query
    ...
    async def test_get_account_notes_with_filters(self, ad_account_service, mock_db, sample_note):
        """测试带过滤条件的备注查询"""
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            mock_query = Mock()
            mock_query.filter.return_value = mock_query
    ...
    async def test_create_account_note_success(self, ad_account_service, mock_db):
        ...
        with patch.object(ad_account_service, 'get_account_by_id', new=AsyncMock()):
            ...
            note = await ad_account_service.create_account_note(1, request, current_user_id=100)
...
class TestDeleteAccount:
    ...
    async def test_delete_account_archived_status(self, ad_account_service, mock_db, sample_account):
        """测试删除archived状态的账户"""
        sample_account.status = "archived"
    ...
    async def test_delete_account_not_archived(self, ad_account_service, mock_db, sample_account):
...
class TestAdAccountServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_account_lifecycle_workflow(self, ad_account_service, mock_db, sample_account):
        """测试账户完整生命周期工作流"""
