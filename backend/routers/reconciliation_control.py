...
async def list_settlement_rules(
    ...
    return success_response(
        data={
            "items": [SettlementRuleResponse.model_validate(r).model_dump() for r in rules],
...
async def batch_create_balance_snapshots(
    ...
    return success_response(
        data={
            ...
            "items": [BalanceSnapshotResponse.model_validate(s).model_dump() for s in snapshots]
        },
...
async def list_issues(
    ...
    return success_response(
        data={
            "items": [ReconciliationIssueResponse.model_validate(i).model_dump() for i in issues],
            "meta": {"total": total, "skip": skip, "limit": limit}
        }
    )
...
async def check_sla_breach(
    ...
    return success_response(
        data={"breached_count": count},
        message=f"已标记 {count} 个SLA超时差异单"
    )
