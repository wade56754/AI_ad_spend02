...
from backend.schemas.reports import (
    ReportQueryRequest,
    PerformanceReportItem, PerformanceReportResponse,
    ProfitReportItem, ProfitReportResponse,
    ReconciliationReportItem, ReconciliationReportResponse,
    FinancialSummaryItem, FinancialSummaryResponse,
    DashboardSummary, TrendDataPoint, TrendReportResponse,
    ReportPeriod, ReportGroupBy
)
...
class ReportsService:
    ...
    async def get_performance_report(
        ...
        for row in results:
            ...
            items.append(PerformanceReportItem(
        ...
        summary = {
            ...
            "avg_cpa": str((total_spend / total_leads).quantize(Decimal("0.01"))) if total_leads > 0 else None,
            "project_count": len(set(item.project_id for item in items if item.project_id)),
            "channel_count": len(set(item.channel_id for item in items if item.channel_id)),
        }
    ...
    async def get_profit_report(
        ...
        topup_results = {row.project_id: Decimal(str(row.topup_amount or 0)) for row in topup_query.all()}
        ...
        summary = {
            ...
            "profit_rate": float(total_profit / total_revenue * 100) if total_revenue > 0 else None,
    ...
    async def get_reconciliation_report(
        ...
        status_query = self.db.query(
            ...
            func.coalesce(func.sum(ReconciliationBatch.total_system_spend), 0).label("system_spend"),
            func.coalesce(func.sum(ReconciliationBatch.total_actual_spend), 0).label("actual_spend"),
            func.coalesce(func.sum(ReconciliationBatch.discrepancy), 0).label("discrepancy"),
        ).filter(
        ...
        items = [ReconciliationReportItem(
            ...
            completion_rate=float((completed.count if completed else 0) / total_batches * 100) if total_batches > 0 else 0,
            discrepancy_rate=float(abs(total_discrepancy) / total_system_spend * 100) if total_system_spend > 0 else 0
        )]
    ...
    async def get_financial_summary(
        ...
        for account in accounts:
            ...
            ledger_data = {row.entry_type: Decimal(str(row.total or 0)) for row in ledger_query.all()}
            ...
            items.append(FinancialSummaryItem(
    ...
    async def get_dashboard_summary(self) -> DashboardSummary:
        ...
        pending_reconciliations = self.db.query(func.count(ReconciliationBatch.id)).filter(
            ReconciliationBatch.status == 'pending_review'
        ).scalar() or 0
