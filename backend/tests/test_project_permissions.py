...
class TestProjectPermissions:
    ...
    async def test_admin_full_permissions(self, async_client, admin_token):
        ...
        # 管理员可以创建项目
        create_data = {"name": "管理员项目", "client_name": "客户", "client_company": "公司"}
        response = await async_client.post(
            "/api/v1/projects", json=create_data, headers=headers
        )
        ...
        response = await async_client.get(
            "/api/v1/projects/statistics", headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_finance_read_only_permissions(self, async_client, finance_token):
        ...
        # 财务不能创建项目
        create_data = {"name": "财务项目", "client_name": "客户", "client_company": "公司"}
        response = await async_client.post(
            "/api/v1/projects", json=create_data, headers=headers
        )
    ...
    async def test_data_operator_permissions(self, async_client, data_operator_token):
        ...
        # 数据员不能创建项目
        create_data = {"name": "数据员项目", "client_name": "客户", "client_company": "公司"}
        response = await async_client.post(
            "/api/v1/projects", json=create_data, headers=headers
        )
        assert response.status_code == 403
    ...
    async def test_account_manager_limited_permissions(
        ...
        # 账户管理员不能创建项目
        create_data = {"name": "新项目", "client_name": "客户", "client_company": "公司"}
        response = await async_client.post(
            "/api/v1/projects", json=create_data, headers=headers
        )
        assert response.status_code == 403
        ...
        # 账户管理员不能删除项目
        response = await async_client.delete(
            f"/api/v1/projects/{account_manager_project_id}", headers=headers
        )
        assert response.status_code == 403
        ...
        # 账户管理员可以管理自己项目的成员
        member_data = {
            "user_id": "00000000-0000-0000-0000-000000000001",  # 有效 UUID 格式
            "role": "media_buyer",
        }
        response = await async_client.post(
    ...
    async def test_media_buyer_minimal_permissions(
        ...
        # 媒体买家不能创建项目
        create_data = {"name": "媒体买家项目", "client_name": "客户", "client_company": "公司"}
        response = await async_client.post(
            "/api/v1/projects", json=create_data, headers=headers
        )
        assert response.status_code == 403
        ...
        # 媒体买家不能删除项目
        response = await async_client.delete(
            f"/api/v1/projects/{media_buyer_project_id}", headers=headers
        )
        assert response.status_code == 403
        ...
        # 媒体买家可以查看自己参与的项目成员
        response = await async_client.get(
            f"/api/v1/projects/{media_buyer_project_id}/members", headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_cross_project_access_denied(
        ...
        update_data = {"status": "active"}
        response = await async_client.put(
            "/api/v1/projects/10000", json=update_data, headers=headers
        )
        # 项目不存在返回 404，权限不足返回 403，参数错误返回 400
        assert response.status_code in [400, 404, 403]
        ...
        response = await async_client.post(
            "/api/v1/projects/10000/members", json=member_data, headers=headers
        )
        # 项目不存在返回 404，权限不足返回 403，参数错误返回 400
        assert response.status_code in [400, 404, 403]
    ...
    async def test_unauthenticated_access(self, async_client):
        """测试未认证访问被拒绝"""
        # 未认证不能创建项目
        create_data = {"name": "项目", "client_name": "客户", "client_company": "公司"}
    ...
    async def test_rls_isolation(self, async_client, admin_token, media_buyer_token):
        ...
        create_data = {"name": "私密项目", "client_name": "私密客户", "client_company": "私密公司"}
        response = await async_client.post(
            "/api/v1/projects", json=create_data, headers=headers_admin
        )
        assert response.status_code == 201
        project_id = response.json()["data"]["id"]
        ...
        response = await async_client.get(
            f"/api/v1/projects/{project_id}", headers=headers_buyer
        )
        assert response.status_code in [403, 404]

    @pytest.mark.asyncio
    async def test_data_filtering_by_role(
        self, async_client, admin_token, account_manager_token
    ):
        """测试基于角色的数据过滤"""
        # 管理员查看所有项目
        headers_admin = {"Authorization": f"Bearer {admin_token}"}
        ...
        response_manager = await async_client.get(
            "/api/v1/projects", headers=headers_manager
        )
