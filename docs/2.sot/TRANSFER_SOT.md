...
**引用**: BUSINESS_RULES.md v4.1 - BR-ACCT-002
...
| 表名 | 职责 | 主键 | 说明 |
|-----|------|------|------|
| **transfer_requests** | 迁移申请记录 | BIGSERIAL | 存储迁移申请状态、金额（待DATA_SCHEMA补充） |
| **ad_accounts** | 广告账户 | BIGSERIAL | 源/目标账户信息（已定义于DATA_SCHEMA 3.2.9节） |
| **suppliers** | 供应商 | UUID | 供应商主数据（待DATA_SCHEMA补充） |
| **ledger_entries** | 资金总账 | BIGSERIAL | TRANSFER_OUT/IN 记录（已定义于DATA_SCHEMA 3.4.4节） |
...
**引用**: BUSINESS_RULES.md v4.1 - BR-ACCT章节
...
**验证逻辑**:
- 检查 `source_account.status == "dead"`, 否则抛出 `E-TRANS-002`
- 检查 `source_account.remaining_balance > 0`, 否则抛出 `E-TRANS-006`
...
**验证逻辑**:
- 检查 `target_account.status == "active"`, 否则抛出 `E-TRANS-003`
- 检查 `source_account.id != target_account.id`, 否则抛出 `E-TRANS-001`
...
**验证逻辑**:
- 检查 `source_account.supplier_id == target_account.supplier_id`, 否则抛出 `E-TRANS-004`
...
**验证逻辑**:
- 检查 `transfer_amount > 0`, 否则抛出 `E-TRANS-006`
- 检查 `transfer_amount <= source_account.remaining_balance`, 否则抛出 `E-TRANS-006`
...
**审批流程**:
1. 使用 `SELECT FOR UPDATE` 锁定 transfer_request 记录
2. 验证 `version == expected_version`, 否则抛出并发冲突错误
3. 更新状态为 `approved`, 设置 `approved_by`, `approved_at`
4. 版本号自增: `version = version + 1`
5. 提交事务
...
| **SELECT FOR UPDATE** | 锁定源/目标账户 + 供应商记录 |
| **原子性保证** | 任一步骤失败，完整回滚 |

**事务流程**:
1. 锁定源账户 (SELECT FOR UPDATE)
2. 锁定目标账户 (SELECT FOR UPDATE)
3. 锁定供应商记录 (SELECT FOR UPDATE)
4. 生成 TRANSFER_OUT Ledger 记录 (负数)
5. 生成 TRANSFER_IN Ledger 记录 (正数)
6. 更新源账户: `remaining_balance = 0`
7. 更新目标账户: `remaining_balance += transfer_amount`
8. 提交事务
...
**执行流程**:
1. 使用 `SELECT FOR UPDATE` 锁定 transfer_request 记录
2. 幂等性检查: 若状态已为 `completed`, 直接返回(幂等)
3. 验证状态必须为 `approved`, 否则抛出 `E-TRANS-005`
4. 执行迁移逻辑 (见第8章)
5. 更新状态为 `completed`, 设置 `completed_at`
6. 提交事务
...
**幂等性保证逻辑**:
1. 生成唯一 `request_no`: `TRF-{YYYYMMDD}-{HHmmss}-{random6chars}`
2. 检查数据库是否存在相同 `request_no`
3. 若存在则返回已有记录(幂等)
4. 若不存在则创建新申请,状态初始化为 `draft`
...
**锁定策略**:
1. 锁定源账户 (使用 `SELECT FOR UPDATE`)
2. 锁定目标账户 (使用 `SELECT FOR UPDATE`)
3. 锁定供应商记录 (使用 `SELECT FOR UPDATE`)

**为什么锁定 supplier**:
- 防止并发迁移同一供应商的不同账户
- 确保供应商余额更新原子性(若DATA_SCHEMA中定义了supplier.balance字段)
...
**超时处理**:
- 设置事务超时: `SET LOCAL statement_timeout = '10s'`
- 设置锁等待超时: `SET LOCAL lock_timeout = '3s'`
- 若超时则抛出 `E-CONC-001` 错误码
...
| 错误码 | 触发条件 | 错误消息示例 |
|-------|---------|------------|
| E-TRANS-002 | `source_account.status != "dead"` | 源账户必须为 dead 状态才能迁移余额 |
| E-TRANS-004 | `source.supplier_id != target.supplier_id` | 禁止跨供应商迁移余额，必须拆分为退款 + 充值 |
| E-CONC-001 | `version != expected_version` 或锁超时 | 版本冲突，请重新获取最新数据 |
...
**权限验证逻辑**:
1. 检查用户角色必须为 `account_manager` 或 `admin`, 否则抛出 `AUTH_500`
2. 若角色为 `account_manager`, 额外检查源账户所属项目的 `account_manager_id` 必须为当前用户, 否则抛出 `AUTH_500` (数据范围限制)
...
**SOD 检查逻辑**:
1. 检查审批人角色必须为 `finance` 或 `admin`, 否则抛出 `AUTH_500`
2. 检查 `transfer_request.created_by != user["user_id"]`, 否则抛出 `BIZ_001` (发起人不能审批)
