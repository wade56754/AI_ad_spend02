...
@require_role(["admin", "finance", "project_owner", "account_manager", "pitcher"])
async def detect_account_anomalies(
    ...
    try:
        anomalies = await service.detect_account_performance_anomalies(
            account_id=account_id, days=days, threshold=threshold
        )
        ...
        return success_response(
            data={
                ...
                "analysis_config": {"days": days, "threshold": threshold},
            },
            message="异常检测完成",
        )
...
@require_role(["admin", "finance", "project_owner", "account_manager"])
async def batch_detect_anomalies(
    ...
    try:
        ...
        return success_response(data=results, message="批量异常检测完成")
...
@router.get(
    "/anomaly-summary", response_model=StandardResponse[dict], summary="获取异常检测摘要"
)
@require_role(["admin", "finance", "project_owner", "account_manager"])
async def get_anomaly_summary(
    ...
    try:
        ...
        return success_response(data=summary, message="获取异常摘要成功")
...
async def assess_account_risk(
    ...
    try:
        ...
        return success_response(data=risk_assessment, message="风险评估完成")
...
async def assess_project_risk(
    ...
    try:
        # 获取项目下的所有账户风险
        results = await service.batch_detect_anomalies(
            project_ids=[project_id], days=days
        )
        ...
        return success_response(
            data={
                ...
                "account_summary": {
                    ...
                    "low_risk_accounts": total_accounts
                    - high_risk_accounts
                    - medium_risk_accounts,
                },
                "anomaly_summary": {
                    "total_anomalies": total_anomalies,
                    "avg_anomalies_per_account": total_anomalies / total_accounts
                    if total_accounts > 0
                    else 0,
                },
...
async def get_ai_recommendations(
    ...
    try:
        # 获取异常检测结果
        anomalies = await service.detect_account_performance_anomalies(
            account_id=account_id, days=days, threshold=1.5  # 使用较低的阈值以获取更多建议
        )
            ...
            recommendations.append(
                {
                    ...
                    "actions": ["设置日预算上限", "检查CPA是否在可接受范围内", "暂停低效广告组", "优化出价避免过度竞价"],
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "actions": ["A/B测试新的广告创意", "优化广告标题和描述", "更新图片或视频素材", "检查目标受众相关性"],
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "actions": ["优化落地页体验", "检查转化追踪是否正常", "调整目标受众定位", "改进行动号召(CTA)"],
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "actions": ["暂停高CPA的广告组", "优化定位以提高转化质量", "改进广告素材相关性", "调整出价策略"],
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "actions": ["定期测试新广告创意", "监控竞品动态", "探索新的受众群体", "保持ROI稳定增长"],
                }
            )
...
@router.get("/insights", response_model=StandardResponse[dict], summary="获取 AI 洞察")
async def get_insights(
    ...
    try:
        ...
        insights = {
            ...
            "highlights": [
                {"type": "positive", "metric": "CPA", "message": "CPA 持续下降，成本控制良好"},
                {"type": "attention", "metric": "spend", "message": "消耗增长较快，注意预算控制"},
            ],
        ...
        return success_response(data=insights, message="获取 AI 洞察成功")
...
@router.get("/anomalies", response_model=StandardResponse[dict], summary="获取异常检测结果")
@require_role(["admin", "project_owner"])
async def get_anomalies(
    ...
    try:
        ...
        anomalies = {
            "total_anomalies": 5,
            "anomalies_by_severity": {"high": 1, "medium": 2, "low": 2},
            "anomalies_by_type": {
                "spend_spike": 1,
                "spend_drop": 1,
                "ctr_drop": 1,
            ...
            "analysis_config": {"days": days, "threshold": 2.0},
            "generated_at": datetime.now().isoformat(),
        }
        ...
        if severity:
            anomalies["anomaly_list"] = [
                a for a in anomalies["anomaly_list"] if a["severity"] == severity
            ]
        ...
        return success_response(data=anomalies, message="获取异常检测结果成功")
...
@router.get("/forecasts", response_model=StandardResponse[dict], summary="获取预测数据")
...
async def get_forecasts(
    ...
    try:
        ...
        result = {
            ...
            "summary": {
                "total_predicted": round(
                    sum(f["predicted_value"] for f in forecasts), 2
                ),
                "avg_daily": round(
                    sum(f["predicted_value"] for f in forecasts) / days_ahead, 2
                ),
                "trend": "increasing",
                "growth_rate_percent": round(
                    (
                        forecasts[-1]["predicted_value"]
        ...
        return success_response(data=result, message="获取预测数据成功")
...
@router.post("/predict", response_model=StandardResponse[dict], summary="AI 预测")
@require_role(["admin", "project_owner"])
async def create_prediction(
    ...
    try:
        ...
        # 生成预测结果
        base_values = {"spend": 1000.0, "conversions": 30, "cpa": 33.33, "roas": 2.5}
        ...
        return success_response(data=result, message="AI 预测完成")
...
@router.post("/optimize", response_model=StandardResponse[dict], summary="AI 优化建议")
@require_role(["admin", "project_owner"])
async def create_optimization(
    ...
    try:
        ...
        result = {
            ...
            "risk_assessment": {
                "level": "low",
                "potential_issues": ["短期内转化量可能略有下降", "需要持续监控效果"],
            },
            "created_by": current_user.id,
            "created_at": datetime.now().isoformat(),
        }
        ...
        logger.info(
            f"AI 优化建议生成完成: {result['optimization_id']} by user {current_user.id}"
        )

        return success_response(data=result, message="AI 优化建议生成成功")
...
@router.get("/recommendations", response_model=StandardResponse[dict], summary="获取推荐")
async def get_recommendations(
    account_id: Optional[int] = Query(None, description="广告账户ID"),
    project_id: Optional[int] = Query(None, description="项目ID"),
    category: Optional[str] = Query(
        None, description="推荐类别: budget, creative, targeting, bidding"
    ),
    ...
    try:
        all_recommendations = [
            {
                ...
                "confidence": 0.85,
                "actions": ["减少广告组 A 预算 30%", "增加广告组 B 预算 50%"],
            },
            {
                "id": "REC-002",
                "category": "creative",
        ...
        if category:
            all_recommendations = [
                r for r in all_recommendations if r["category"] == category
            ]
        ...
        return success_response(data=result, message="获取推荐成功")
