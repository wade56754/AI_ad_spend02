...
# -*- coding: utf-8 -*-
"""
时间戳注入 Hook - SessionStart 时注入北京时间上下文
让 Claude 了解当前时间、日期、工作日等信息
用法:
    作为 SessionStart hook 自动运行
    或手动运行: python inject_timestamp.py
"""
...
def get_beijing_time() -> dict:
    ...
    return {
        ...
        "period_emoji": emoji,
        "season": season,
        "display_cn": f"{now.strftime('%Y年%m月%d日')} {weekdays_cn[weekday_idx]} {period} {now.strftime('%H:%M')}",
    }


def main():
    time_info = get_beijing_time()
    workday_icon = "💼" if time_info["is_workday"] else "🏖️"
