...
# 检查 Node.js 和 pnpm
echo -e "${YELLOW}1. 检查 Node.js 和 pnpm...${NC}"
...
for dep in "${REQUIRED_DEPS[@]}"; do
    if pnpm list --depth 0 "$dep" | grep -q "$dep@"; then
        echo -e "${GREEN}✓ $dep${NC}"
    else
...
if [ "$ALL_DEPS_OK" = false ]; then
    echo -e "${RED}请运行 pnpm install 安装依赖${NC}"
    exit 1
fi
...
if pnpm test -- __tests__/setup.test.ts --silent; then
...
if pnpm test -- __tests__/example/ --silent; then
