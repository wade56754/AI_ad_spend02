...
async def create_topup(
    ...
    current_user: User = Depends(
        require_role(["pitcher", "account_manager"])  # v5.0: 使用 pitcher
    ),
):
    """创建充值申请"""
    try:
...
async def get_statistics(
    ...
    service: TopupService = Depends(get_topup_service),
    current_user: User = Depends(
        require_role(["admin", "finance", "ceo"])  # v5.0: 移除 data_operator
    ),
):
    """获取充值统计详情"""
    try:
...
async def reject_topup(
    ...
    current_user: User = Depends(
        require_role(
            ["finance", "admin", "ceo", "project_owner"]
        )  # v5.0: 移除 data_operator
    ),
):
    """
    拒绝充值申请
...
async def review_topup(
    ...
    service: TopupService = Depends(get_topup_service),
    current_user: User = Depends(
        require_role(["finance", "project_owner", "admin"])
    ),  # v5.0: 替换废弃的 data_operator/supervisor
):
    """数据员审核"""
    try:
        client_ip, user_agent = get_client_info(req) if req else (None, None)
