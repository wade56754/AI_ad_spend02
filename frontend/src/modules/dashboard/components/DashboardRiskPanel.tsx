      ...
      <CardHeader className="pb-3">
        <div className="flex justify-between items-center">
          ...
          {criticalCount > 0 && (
            <Badge
              variant="destructive"
              className="text-xs font-semibold animate-pulse bg-danger/20 text-danger border-danger/40"
            >
              {alerts.length} 待处理
            </Badge>
          )}
            ...
            {alerts.map((alert) => (
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-xs text-text-muted flex-1">{alert.msg}</span>
                </div>
                {alert.timestamp && (
                  <div className="text-[10px] text-text-subtle mt-1.5 ml-3">
                    {alert.timestamp}
                  </div>
                )}
              </div>
