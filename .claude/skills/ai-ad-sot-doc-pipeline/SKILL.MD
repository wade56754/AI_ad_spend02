...
description: >
  ...
  使用 SuperClaude 角色系统（Planner / Executor / Critic / Summarizer）和六阶段命令流，
  按固定四阶段（analyze → cleanup → design → implement）执行 /sc 命令。
  仅在用户或上层 orchestrator 明确给出指令时运行，不自动发起任何审查、修复或代码操作。
version: "3.0"
---
...
1. 用户在对话中明确下达任务，例如：  
   - “请运行一次 analyze + cleanup 流水线，目标是 docs/2.sot/*.md”
...
**功能**：  
- 按调用方指定的路径扫描 SoT 文档（如 `docs/2.sot/*.md`）  
- 提取结构信息（章节、索引、引用等）  
...
阶段：["analyze", "cleanup"]
目标：
- analyze: docs/2.sot/*.md
- cleanup: ["docs/2.sot/DAILY_REPORT_SOT.md", "docs/2.sot/LEDGER_SOT.md"]
约束：
...
- analyze：
  ...
  - 主要发现：缺失 SOT、标题层级不一致…
- cleanup：
  - 修改文件：["docs/2.sot/DAILY_REPORT_SOT.md", ...]
...
复制代码
{
  ...
  "cleanup": {
    "touched_files": ["docs/2.sot/DAILY_REPORT_SOT.md"],
    "safe_mode": true
  },
  "warnings": [],
