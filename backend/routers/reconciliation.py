...
from backend.models import ReconciliationBatch, ReconciliationDetail, ReconciliationAdjustment
    ...
    try:
        ...
        return success_response(
            data={
                ...
                "adjustment": ReconciliationAdjustmentResponse.model_validate(adjustment)
            },
            message="明细已调整"
        )
...
async def update_batch(
    ...
    - data_operator: 仅可更新自己创建的草稿批次
    """
    try:
        batch = await service.update_batch(batch_id, request, current_user.id, current_user.role)
...
async def get_detail_by_id(
    ...
    """获取单个对账明细详情"""
    try:
        detail = await service.get_detail_by_id(detail_id, current_user.id, current_user.role)
...
async def get_adjustments(
    ...
    adjustment_type: Optional[str] = Query(None, description="调整类型: increase/decrease/writeoff"),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_role(["admin", "finance"])),
    service: ReconciliationService = Depends(get_reconciliation_service)
):
    ...
    try:
        ...
        adjustment_responses = [
            ReconciliationAdjustmentResponse.model_validate(adj)
            for adj in adjustments
        ]

        return paginated_response(
            items=adjustment_responses,
            total=total,
            page=page,
            page_size=page_size
        )
