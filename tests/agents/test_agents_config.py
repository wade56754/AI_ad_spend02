...
def test_list_agents_contains_core_agents():
    """Test that list_agents returns all four core agents."""
    agents = list_agents()
    assert "fe" in agents
    assert "be" in agents
    assert "test" in agents
    assert "orch" in agents
    assert agents["fe"]["key"] == "fe"
    assert agents["be"]["key"] == "be"
    assert agents["test"]["key"] == "test"
    assert agents["orch"]["key"] == "orch"


def test_create_fe_agent_has_handle_request():
    """Test that created FEAgent has handle_request method."""
    fe_agent = create_agent("fe")
    assert hasattr(fe_agent, "handle_request")
    assert callable(fe_agent.handle_request)


def test_create_agent_invalid_name_raises():
    """Test that create_agent raises KeyError for invalid agent name."""
