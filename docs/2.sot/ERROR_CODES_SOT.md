...
| `BIZ_` | 业务逻辑 | 业务规则验证、状态转换、账本操作等错误 | 400, 404, 409, 500 | 18 |
...
| 错误码 | 消息 | HTTP | 触发场景 | 状态 |
|--------|------|------|----------|------|
| `BIZ_600` | 账本创建失败 | 500 | 创建账本分录时数据库操作失败 | USED |
| `BIZ_601` | 账本查询失败 | 500 | 查询账本记录时发生异常 | USED |
| `BIZ_602` | 交易记录不存在 | 404 | 根据ID查询交易记录未找到 | USED |
| `BIZ_603` | 账本更新失败 | 500 | 更新账本记录时数据库操作失败 | USED |
| `BIZ_604` | 余额查询失败 | 500 | 查询账户余额时发生异常 | USED |
| `BIZ_605` | 预算查询失败 | 500 | 查询预算分配时发生异常 | USED |
| `BIZ_606` | 预算创建失败 | 500 | 创建预算分配时数据库操作失败 | USED |
| `BIZ_607` | 统计查询失败 | 500 | 获取交易统计信息时发生异常 | USED |
...
| v2.2 | 2025-12-07 | **【账本错误码登记】**<br>• 新增 BIZ_600~607 账本相关错误码（8个）: LEDGER_CREATE_ERROR/LEDGER_QUERY_ERROR/TRANSACTION_NOT_FOUND/LEDGER_UPDATE_ERROR/BALANCE_QUERY_ERROR/BUDGET_QUERY_ERROR/BUDGET_CREATE_ERROR/STATISTICS_QUERY_ERROR<br>• 更新 §2.2 类别前缀表: BIZ_ 数量 10 → 18<br>• 更新 §2.3 BIZ_ 编码分段: 新增 600-699 账本相关段<br>• 更新错误码统计: 59个 → 67个总数<br>• 更新 HTTP 500 错误码数量: 7 → 14<br>• 理由: 代码已使用但 SoT 未登记的错误码规范化补录 | Error Code Hygiene Agent |
